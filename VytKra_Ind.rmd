...
summary(holtwinters_ts)

accuracy(holtwinters_ts)
```
...
```{r}
accuracy_apmokymo <- lapply(modeliai, accuracy)
accuracy_apmokymo  <- do.call(rbind.data.frame, accuracy_apmokymo) %>% 
   rownames_to_column("Metodas") %>% 
   select(Metodas, MAE)
   

accuracy_testavimo <- lapply(modeliai, accuracy, test = testavimo)
accuracy_testavimo <- do.call(rbind.data.frame, accuracy_testavimo) %>% 
   rownames_to_column("Metodas") %>% 
   select(Metodas, MAE)
```

```{r}
tikslumai <- accuracy_apmokymo %>% 
   left_join(accuracy_testavimo, by = c("Metodas"), suffix = c("_apmokymo", "_testavimo"))
```
